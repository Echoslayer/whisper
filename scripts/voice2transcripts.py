...
# 使用 Whisper.cpp 進行音訊轉錄的輔助函數
def transcribe_single_clip(args):
    """Helper function to transcribe a single audio clip."""
    clip_filename, start_time, end_time, whisper_exec, whisper_model, language, use_coreml, clip_index, total_clips = args
    print(f"🎤 轉錄片段 {clip_index}/{total_clips}: {os.path.basename(clip_filename)} ...")
    coreml_model_path = whisper_model.replace(".bin", ".mlmodelc") if use_coreml else whisper_model
    cmd = [whisper_exec, "-m", coreml_model_path if use_coreml else whisper_model,
    ...
    try:
        ...
        print(f"✅ 片段 {clip_index}/{total_clips} 轉錄完成")
        return f"{timestamp}\n{text}\n\n"
    except Exception as e:
