    ...
    with open(transcript_file, "w", encoding="utf-8") as f_txt:
        ...
        for i, (clip_filename, start_time, end_time) in enumerate(clip_files, 1):
            ...
            cmd = [whisper_exec, "-m", whisper_model if not use_coreml else coreml_model_path,
                   "-f", clip_filename, "-l", language, "-otxt", "-of", clip_filename]
            ...
            try:
                ...
                # Read the output text file generated by Whisper.cpp
                text_file = clip_filename + ".txt"
                text = ""
                if os.path.exists(text_file):
                    with open(text_file, "r", encoding="utf-8") as tf:
                        text = tf.read().strip()
                    os.remove(text_file)  # Clean up the temporary text file
